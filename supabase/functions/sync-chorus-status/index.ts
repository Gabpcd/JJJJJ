...
/** Auth service_role : décoder JWT et vérifier role */
function verifyServiceRole(authHeader: string | null): { ok: boolean; error?: string } {
  if (!authHeader?.startsWith('Bearer ')) return { ok: false, error: 'Authorization Bearer manquant' };
  const token = authHeader.slice(7).trim();
  const parts = token.split('.');
  if (parts.length !== 3) return { ok: false, error: 'JWT format invalide' };
  try {
    const b64 = parts[1].replace(/-/g, '+').replace(/_/g, '/');
    const padded = b64 + '='.repeat((4 - b64.length % 4) % 4);
    const payload = JSON.parse(atob(padded));
    if (payload?.role !== 'service_role') return { ok: false, error: `Role '${payload?.role}' non autorisé` };
    return { ok: true };
  } catch (e) {
