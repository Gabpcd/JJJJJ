...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (numeroRpps.startsWith(TEST_PREFIX) && testModeActif) {
      ...
      if (!testRow) {
        return new Response(JSON.stringify({ trouve: false, correspond: false, nom_api: null, prenom_api: null, profession_api: null, source: 'Mode test (rpps_test)' }),
    ...
    try {
      ...
      if (!result.trouve) {
        return new Response(JSON.stringify({ trouve: false, correspond: false, nom_api: null, prenom_api: null, profession_api: null, source: 'FHIR Annuaire Sante' }),
          { headers: { ...corsHeaders(req), 'Content-Type': 'application/json' } });
      }
      ...
      if (soignant_id && correspond) {
        ...
        } catch (dbErr) { console.error('Erreur sauvegarde RPPS sur soignant:', dbErr); }
      }
      return new Response(JSON.stringify({
