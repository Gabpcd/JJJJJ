-- Le staging avait déjà enregistré le rollback 20260810170533 avant que sa
-- régression soit détectée. Cette migration compensatrice rétablit de façon
-- idempotente l'état sûr déjà présent en production : classement borné,
-- helper de test réservé au service_role et empreintes basées sur prosrc.
BEGIN;
...
BEGIN
  ...
  JOIN pg_catalog.pg_proc p ON p.oid = r.procedure_oid
  WHERE pg_catalog.md5(p.prosrc) <> i.definition_md5;
