-- PR 2 Sprint 1 — Matrice de compatibilité exercice × profession × type étab
--
-- Contexte : aujourd'hui rien n'empêche un IDE libéral de candidater à une
-- mission LIBERAL en EHPAD ou clinique privée. C'est un cas de salariat
-- déguisé (art. L8221-1 Code travail, Conseil d'Etat 11/02/2025 arrêt
-- Mediflash). Il faut une matrice qui pour chaque profession × type étab
-- autorise ou non l'exercice libéral.
--
-- Stratégie : ADD VALUES au type_etablissement existant pour les 8 cabinets
-- libéraux dédiés. Créer une RPC `peut_exercer_liberal(profession, type_etab)`
-- qui encode la matrice. Wire la validation dans fn_postuler_mission et
-- au niveau trigger pour défense en profondeur.
...
-- Note : ALTER TYPE ADD VALUE must run outside the transaction with the
-- function definition that uses the new values (Postgres limitation). On
-- utilise SECURITY DEFINER + text params (pas l'enum) pour éviter le
-- problème.
...
BEGIN
  ...
  RETURN CASE p_profession
    ...
    -- AS, AES, IBODE, IADE, PHARMACIEN, PREPARATEUR_PHARMA, MANIPULATEUR_RADIO,
    -- DIETETICIEN, AUXILIAIRE_PUERICULTURE : pas de mode libéral autorisé.
    ELSE FALSE
  END;
END;
$function$;
...
-- 3. RPC peut_exercer (wrapper qui couvre aussi le mode salarié)
...
AS $function$
BEGIN
  -- Salarié : compatible avec tous types d'établissement (par défaut).
  -- Une matrice plus fine peut être ajoutée plus tard (ex : un IDE en
  -- pharmacie officine n'a pas vraiment de sens) mais hors scope v1.
  IF p_type_exercice IN ('SALARIE', 'CDD', 'CDDU', 'VACATION') THEN
    RETURN TRUE;
  END IF;

  -- Libéral / MIXTE : on délègue à la matrice libérale.
  IF p_type_exercice IN ('LIBERAL', 'MIXTE') THEN
    RETURN public.peut_exercer_liberal(p_profession, p_type_etablissement);
  END IF;

  -- Type d'exercice inconnu : par défaut autoriser pour ne pas casser
  -- de cas marginal. Logger côté frontend pour investigation.
  RETURN TRUE;
END;
$function$;
...
-- Défense en profondeur — le frontend désactive aussi visuellement l'option.
...
DECLARE
  v_type_etab text;
BEGIN
  -- Ne vérifier que si la mission cible explicitement le mode libéral
  -- (extraction depuis la description tag [CONTRAT:LIBERAL] ou colonne
...
INSERT INTO public.journaux_audit (
  ...
  'MATRICE_COMPATIBILITE_LIBERAL_INSTALLED', 'fonction', NULL,
  jsonb_build_object(
    'pr', 'PR 2 Sprint 1 révisé',
