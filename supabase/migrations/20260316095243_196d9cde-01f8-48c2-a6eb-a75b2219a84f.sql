...
BEGIN
  SELECT jsonb_build_object(
    ...
    -- CA potentiel = toutes missions terminées (même sans contrat)
    'ca_potentiel_mois', COALESCE((
      SELECT sum(montant_commission_ht) FROM missions WHERE statut = 'TERMINEE' AND modifie_le >= debut_mois
    ), 0),
    'ca_potentiel_total', COALESCE((
      SELECT sum(montant_commission_ht) FROM missions WHERE statut = 'TERMINEE'
    ), 0),
