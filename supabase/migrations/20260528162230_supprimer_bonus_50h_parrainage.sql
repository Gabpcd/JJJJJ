-- Fix : suppression du bonus +50h heures_cumulees du parrainage
--
-- Le +50h faussait les heures réellement travaillées. Illégal pour le
-- seuil 3 200h passage libéral. Seule la prime cash 50€ reste.
--
-- Remplace fn_trg_valider_parrainage_soignant_premiere_mission sans le
-- UPDATE soignants SET heures_cumulees. Supprime aussi le +50h du trigger
-- legacy fn_appliquer_parrainage (qui était déjà désactivé Sprint iter1).
...
BEGIN
  ...
  WHERE filleul_id = NEW.soignant_assigne_id AND statut = 'EN_ATTENTE' LIMIT 1;

  IF v_parrainage IS NOT NULL THEN
    SELECT EXISTS(
      SELECT 1 FROM missions WHERE soignant_assigne_id = v_parrainage.parrain_id AND statut = 'TERMINEE' LIMIT 1
    ...
    IF v_parrain_a_mission THEN
      ...
      IF v_nb_filleuls_actifs < 20 THEN
        ...
        VALUES (
          ...
          'Filleul activé !',
          'Votre filleul a terminé sa 1ère mission. La prime de 50€ sera versée après 100€ de commission encaissée.',
          '/soignant/parrainage'
        );
  ...
  LOOP
    ...
    IF v_nb_filleuls_actifs < 20 THEN
      ...
      VALUES (
        ...
        'Filleul activé !',
        'Votre filleul a terminé sa 1ère mission.',
        '/soignant/parrainage'
      );
    END IF;
  END LOOP;
