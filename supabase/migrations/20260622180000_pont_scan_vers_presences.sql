-- Pointage rotatif — PONT vers l'ancien système presences (option 1).
--
-- Contexte : 25 fonctions (validation, score, litige, no-show, facture, RGPD…)
-- lisent la table `presences`. Le système rotatif écrit dans `scans_pointage` +
-- créneaux EFFECTIF, donc déconnecté de toute cette machinerie.
--
-- Solution non-destructive : à chaque scan, fn_scanner_code_pointage maintient
-- AUSSI une ligne `presences` « résumé » :
--   - 1ʳᵉ OUVERTURE → crée la présence (arrivée) + passe la mission EN_COURS
--     (comme l'ancien fn_pointer_arrivee) → no-show / score / anti-triche OK.
--   - chaque FERMETURE → met à jour départ = dernière fermeture + heures_reelles
--     = somme des segments EFFECTIF travaillés → facture / score lisent juste.
--
-- Les 25 fonctions continuent de marcher SANS modification. Le multi-jours et les
-- pauses restent gérés finement par les créneaux EFFECTIF (source de vérité paie
-- via fn_calculer_montant_periode). Le trigger trg_protect_presence_integrity ne
-- gêne pas (il protège seulement les champs anti-fraude, qu'on ne touche pas).
