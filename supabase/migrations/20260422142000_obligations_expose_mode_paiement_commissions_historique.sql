-- Phase 2 Option Y — exposer `mode_paiement`, `virement_reference`,
-- `stripe_payment_intent_id`, `mission_id` dans `factures_commission_historique`
-- retourné par fn_obligations_financieres, afin d'afficher le mode de paiement
-- ("Stripe à la source" / "Virement" / "Chorus Pro") dans l'UI étab.
--
-- Aussi : `missions_non_facturees` exclut désormais les missions avec facture
-- commission existante (même exclusion que le cron mensuel — évite l'affichage
-- erroné "à facturer" sur les missions payées via Stripe Connect).
