-- ============================================================
-- BUG-UI-OBLIG-1 Fix #1 — fn_obligations_financieres : filtre RESOLU
-- ============================================================
-- Problème : missions avec paiement soignant statut RESOLU (litige
-- clos) ré-apparaissent dans "Missions à payer aux soignants" car
-- le filtre NOT EXISTS ne couvrait que DECLARE + CONFIRME.
--
-- Fix : élargir le filtre à DECLARE, CONFIRME, RESOLU.
-- - DECLARE  : paiement déclaré, en attente de confirmation soignant
-- - CONFIRME : paiement confirmé par le soignant (cas normal)
-- - RESOLU   : litige clos (paiement jugé OK ou compensé)
-- CONTESTE reste visible (litige en cours → mission à retraiter).
-- EN_ATTENTE reste visible (paiement pas encore envoyé).
-- ============================================================
