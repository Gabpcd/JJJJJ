...
| TECH-DEBT-TESTS-RPC | Créer fichiers de tests SQL pour RPCs critiques (fn_obligations_financieres, fn_alerter_paiements_retard, fn_gerer_blocage_etabs, fn_traiter_candidature, fn_auto_transitions_missions, fn_mon_etablissement_complet) : tests en BEGIN/ROLLBACK vérifiant les filtres métier (paiement DECLARE/CONFIRME/RESOLU exclus, CONTESTE visible, etc.). À exécuter avant tout CREATE OR REPLACE. Idéalement intégré au workflow GitHub Actions via hook pre-deploy. Prévient les régressions silencieuses observées sur fn_obligations_financieres (BUG-UI-OBLIG-1) | P1 | OUVERT | Session BUG-UI-OBLIG-1 22/04/2026 — filet anti-régression RPCs critiques | 6 | SP-QUALITY-TESTS-RPC |
| BUG-COMMISSION-DOUBLE-FACTURATION | Suspicion identifiée lors diagnostic CRITIQUE 2 (22/04/2026) : les missions LIBERAL+Stripe Connect (commission capturée à la source par Stripe Checkout) ont quand même `commission_facturee=TRUE` et sont couvertes par une facture commission groupée (ex : mission `d2000000-0000-0000-0000-000000000002` Vaccination 30/03 LIBERAL, apparaît dans facture JOL-202603-0001 période mars). Risque de double facturation de la commission. Auditer le flow de génération factures commission pour exclure les missions dont commission est capturée par Stripe Connect (ou marquer `commission_facturee=TRUE` sans inclure dans facture groupée) | P1 | OUVERT | Session diagnostic CRITIQUE 2 22/04/2026 | 4 | SP-I-commission-flow-hardening |
| BUG-ABSENCE-DECOMPO | Mission `statut = ABSENCE` (ou `ANNULEE`) affiche décomposition financière complète dans DetailMission comme si honorée → induit erreur de paiement potentielle. Audit attendu : lister statuts mission (enum/CHECK), documenter comportement rémunération par statut (attendu : ABSENCE/ANNULEE → aucune rémunération, TERMINEE → rémunération normale, LITIGE → rémunération en instance). Fix attendu : DetailMission + DecompositionFinanciere conditionnent affichage sur statut (ABSENCE/ANNULEE → bloc "Mission non honorée, aucune rémunération"). Gardes backend : aucune facture honoraires ni commission pour ces statuts (generate-invoice + fn_creer_facture_commission_*). Compléter docs/logique-paiements-v1.md avec section "Statuts mission et rémunération". | P0 | RÉSOLU | Constat session 22/04/2026 → fix 22/04/2026 : DecompositionFinanciere early-return pour ABSENCE/ANNULEE_PAR_*/EXPIREE + doc v1 §7 "Statuts mission et rémunération". Audit gardes backend validé : toutes en place (generate-invoice L695, fn_declarer_paiement_soignant x2, stripe-connect-pay-mission L97, fn_auto_facturation_mensuelle filtre TERMINEE, fn_obligations_financieres filtre TERMINEE). | 6 | SP-UI-FIX-CRITIQUES |
| REFONTE-FACTURATION-OBLIGATIONS | Pages `/etablissement/facturation` et `/etablissement/obligations` affichent informations chevauchées (factures commission sur obligations, paiements mission sur facturation, double source) → confusion utilisateur. Proposition : fusion en une page unifiée `/etablissement/finance` avec onglets "À payer" / "Historique" / "Factures commission". Alternative : clarification + liens cross explicites entre les 2 pages. UX à discuter Gabrielle avant codage. | P2 | OUVERT | Constat session 22/04/2026 | 8 | SP-UI-FINANCE-CLARIFICATION |
| BUG-AUDIT-ACTIONS-MANQUANTES | CHECK constraint `journaux_audit_action_check` n'autorise pas les actions Stripe/Connect utilisées en prod : `FINANCE_TRANSFER_CONNECT`, `CONNECT_METADATA_MANQUANTE`, `STRIPE_CHECKOUT_ORPHANED_RECOVERED`, `FACTURE_HONORAIRES_PAYEE_SKIP_ANOMALIE`, `PAIEMENT_MONTANT_ECART` (introduite Fix F 22/04). `fn_ecrire_audit_safe` catch `WHEN OTHERS` avale silencieusement les CHECK violations → ces audits ne sont JAMAIS persistés en prod, masquant la traçabilité RGPD + visibilité ops (0 audit visible pour ces actions dans `journaux_audit` malgré code production les écrivant). Migration pour étendre le CHECK avec les 5 actions ci-dessus. Optionnel : ajouter log-warning explicite dans `fn_ecrire_audit_safe` plutôt que swallow silencieux, pour détecter les régressions futures. | P1 | RÉSOLU | Diagnostic fixtures 22/04/2026 — bug révélé par tentative INSERT action=FINANCE_TRANSFER_CONNECT. Fix 22/04/2026 : migration `20260422110000_bug_audit_actions_manquantes.sql` étend CHECK de 39 → 81 actions (42 ajoutées : 22 Stripe/Connect + 8 RPCs + 12 frontend). Déploiement prioritaire urgent suite blocage diagnostic webhook M2 Stripe Connect (pi_3TOwlQ à 09:08) : les 3 invocations webhook retournaient 200 sans aucun audit visible, impossible d'investiguer. | 2 | SP-OBSERVABILITY-AUDIT-COMPLETENESS |
...
| P0 RÉSOLUS      | 13     | A24, A25, E1, E2, E11, E12, E15, E16, H1, H2, H3, H4, BUG-ABSENCE-DECOMPO |
| P1 OUVERTS      | 27     | A5, A7, A8, A16, D4, D5, D6, D7, D11, D12, F5, F6, F7, F8, F9, F14, G3, K1, L4, L5, UI-1b, UI-2a, UI-2b, BUG-UI-OBLIG-1, BUG-UI-STRIPE-1, TECH-DEBT-TESTS-RPC, BUG-COMMISSION-DOUBLE-FACTURATION |
| P1 EN COURS     | 1      | B4                                                                      |
| P1 RÉSOLUS      | 18     | A4, A20, A21, A23, A26, E5, E6, E7, E13, E14, H5, H6, H7, H8, H13, H14, BUG-UI-EVAL-1, BUG-AUDIT-ACTIONS-MANQUANTES |
| P2 OUVERTS      | 51     | A1, A2, A3, A6, A9, A10, A11, A12, A14, A15, A19, B3b, C1, D8, D9, D10, F10, F11, F12, G4, H15, H16, I2, I3, I4, I5, J1, J2, J3, K2, K3, L6, L7, L8, UI-1c, UI-2c, UI-2d, UI-2e, UI-2f, UI-2g, UI-2h, UI-2i, UI-2j, UI-E16-1, UI-E16-2, UI-C4-email, BUG-RLS-1, UI-C5-badge-etab, BUG-UI-CHORUS-CONFIG, UI-MISSION-FICHE-RICHE, REFONTE-FACTURATION-OBLIGATIONS |
| P2 RÉSOLUS      | 9      | A22, B5, E8, E9, H9, H10, H11, H12, I1                                  |
| DIFFÉRÉS        | 5      | A13, A17, A18, C2, C3                                                   |

**Validation somme** : 21 + 13 + 26 + 1 + 16 + 50 + 9 + 5 = **141** ✓ (BUG-ABSENCE-DECOMPO P0 OUVERT→RÉSOLU le 22/04/2026)
