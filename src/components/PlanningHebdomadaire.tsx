...
const HEURE_MIN = 6;
const HEURE_MAX = 30; // 6h jour suivant
...
export function PlanningHebdomadaire({ missionCandidate }: PlanningHebdomadaireProps) {
  ...
  function getMissionBlocs(jour: Date) {
    ...
    for (const m of msDuJour) {
      ...
      if (heureFin <= heureDebut) heureFin += 24; // Mission de nuit
  ...
  return (
    <div className="card-base">
      ...
      <button
        onClick={() => setSemaine(startOfWeek(new Date(), { weekStartsOn: 1 }))}
          ...
          <div className="grid grid-cols-8 gap-px" style={{ height: '500px' }}>
            ...
            <div className="relative">
              {Array.from({ length: TOTAL_HEURES }, (_, i) => (
                <div
                  key={i}
                  className="absolute text-[9px] text-muted-foreground right-1"
