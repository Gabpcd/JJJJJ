  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-labelledby="wizard-litige-titre"
      >
        <div className="flex items-center justify-between">
          <h2 id="wizard-litige-titre" className="text-lg font-bold text-foreground inline-flex items-center gap-2">
            <AlertTriangle className="h-5 w-5 text-warning" />
            Signaler un problème
          </h2>
          <button onClick={onClose} className="p-1 hover:bg-muted rounded-lg" aria-label="Fermer">
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {TYPES.map((t) => (
                <label
                  key={t.value}
                  className={`flex items-start gap-2 rounded-lg border-2 p-3 cursor-pointer transition-colors ${
                    typeLitige === t.value
