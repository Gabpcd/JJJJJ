...
import { MOCK_SOIGNANT, MOCK_MISSIONS } from '@/lib/mock-data';
import { Soignant } from '@/lib/types';
import { format } from 'date-fns';
import { fr } from 'date-fns/locale';

function calculerCompletionProfil(s: Soignant): { pourcentage: number; manquants: string[]; completes: string[] } {
  const checks: [boolean, string][] = [
    [!!s.prenom, 'Prénom'],
    [!!s.nom, 'Nom'],
    [!!s.telephone, 'Téléphone'],
    [!!s.date_naissance, 'Date de naissance'],
    [!!s.profession, 'Profession'],
    [!!s.type_contrat, 'Type de contrat'],
    [!!(s.numero_rpps || s.numero_adeli), 'Numéro RPPS/ADELI'],
    [!!(s.adresse_lat && s.adresse_lng), 'Adresse géolocalisée'],
    [s.tous_documents_valides, 'Documents validés'],
    [s.identite_verifiee, 'Identité vérifiée'],
  ];
...
export default function DashboardSoignant() {
  ...
  const soignant = MOCK_SOIGNANT;
  const missions = MOCK_MISSIONS;
  const [modalScore, setModalScore] = useState(false);
  const profil = calculerCompletionProfil(soignant);
  const scoreConfig = getScoreConfig(soignant.score_fiabilite);
  ...
  return (
    <LayoutApp role="SOIGNANT">
      ...
      {profil.pourcentage < 100 ? (
        <div className="rounded-2xl bg-gradient-to-r from-primary/5 to-info/5 border border-primary/20 p-4 md:p-6 mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('/soignant/profil')} className="text-xs text-primary font-medium hover:underline">
              Compléter →
            </button>
          </div>
          <JaugeProgression valeur={profil.pourcentage} max={100} couleurBarre="bg-primary" couleurFond="bg-primary/10" />
          <div className="mt-3 grid grid-cols-2 gap-1.5">
            {profil.completes.map(c => (
              <div key={c} className="flex items-center gap-1.5 text-xs text-primary">
                <CheckCircle2 className="h-3.5 w-3.5" />{c}
              </div>
            ))}
            {profil.manquants.map(m => (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <CarteKPI icone={CheckCircle} valeur={soignant.total_missions_terminees} label="Missions terminées" couleurIcone="text-success" couleurFond="bg-success/10" />
        <div className="card-kpi cursor-pointer" onClick={() => setModalScore(true)}>
          <div className="flex items-start gap-3">
            <div className={`rounded-xl p-2.5 ${scoreConfig.couleurFond}`}>
              <Star className={`h-5 w-5 ${scoreConfig.couleurIcone}`} />
            </div>
            <div>
              <p className="text-2xl font-bold text-foreground">{soignant.score_fiabilite}<span className="text-sm text-muted-foreground">/100</span></p>
        ...
        <CarteKPI icone={Clock} valeur={`${soignant.heures_cumulees}h`} label="Heures cumulées" sousLabel="sur 3 200h objectif" couleurIcone="text-purple-600" couleurFond="bg-purple-100" />
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              {soignant.tous_documents_valides ? (
                ...
                <>
                  <span className="badge-base bg-destructive/10 text-destructive">✗ Incomplet</span>
                  <button onClick={() => navigate('/soignant/documents')} className="block text-[10px] text-primary mt-1 hover:underline">Ajouter →</button>
                </>
              )}
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => navigate('/soignant/missions')} className="text-sm text-primary font-medium hover:underline">
            Voir tout →
          </button>
        </div>
      ...
      <div className="rounded-2xl bg-gradient-to-r from-purple-50 to-purple-100/50 border border-purple-200 p-4 md:p-6">
        ...
        <JaugeProgression
          valeur={soignant.heures_cumulees}
          max={3200}
          marqueurs={[800, 1600, 2400, 3200]}
          couleurBarre="bg-gradient-to-r from-purple-500 to-purple-600"
          couleurFond="bg-purple-100"
        />
        <div className="flex justify-between text-[10px] text-muted-foreground mt-1.5">
          <span>0h</span><span>800h</span><span>1600h</span><span>2400h</span><span>3200h</span>
        </div>
        <p className="text-sm font-semibold text-foreground mt-3">
          <span className="text-purple-600">{soignant.heures_cumulees}h</span> / 3 200h
        </p>
