...
export default function InscriptionEtablissement() {
  ...
  const { inscriptionEtablissement, loading } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      afficherNotification({ type: 'erreur', message: 'Erreur lors de la création.' });
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          {etape === 1 && (
            <div className="space-y-4">
              ...
              <button type="button" onClick={() => setEtape(2)} disabled={!etape1Valide} className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed">
                Continuer
              </button>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.siret} onChange={e => maj('siret', e.target.value.replace(/\D/g, '').slice(0, 14))} className="input-base" required />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">FINESS (9 chiffres)</label>
                  <input value={form.finess} onChange={e => maj('finess', e.target.value.replace(/\D/g, '').slice(0, 9))} className="input-base" />
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-foreground mb-1.5 block">Type d'établissement *</label>
                <SelectTypeEtablissement value={form.type} onChange={v => maj('type', v)} />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">Email contact</label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={!etape2Valide || loading} className="btn-primary flex-1 disabled:opacity-50 disabled:cursor-not-allowed">
