-- 7b-C (F4) — la notation 1-tap arrive au check-out (soignante) et à la
-- validation des présences (étab). Ces deux moments peuvent précéder le passage
-- TERMINEE (fn_auto_transitions_missions tourne toutes les 10 min) : une mission
-- EN_COURS dont le départ est pointé est de facto finie → notable.
-- Base : définition live (inclut le blocage pendant litige actif, iter1 B.8).
