-- Correctifs cœur métier candidatures / matching (audit en profondeur 27/06).
-- Tous appliqués en prod via MCP puis enregistrés dans schema_migrations.
--
-- 1. fn_mes_matches      : filtrait candidature.statut IN (ASSIGNEE/EN_COURS/TERMINEE)
--    — des statuts de MISSION qu'une candidature ne prend jamais → page "Mes Matches"
--    toujours vide. Corrigé en c.statut = 'ACCEPTEE' (un match = candidature acceptée).
-- 2. fn_postuler_mission : le bouton "Postuler" classique ne notifiait jamais l'étab
--    (seul le swipe insérait CANDIDATURE_RECUE). Ajout de la notif pour parité.
-- 3. fn_accepter_mission : égalité stricte profession=profession_requise → remplacée par
--    fn_soignant_compatible_mission (compat hiérarchique IBODE/IADE↔IDE, spécialité MEDECIN).
-- 4. fn_traiter_candidature : (a) garde d'idempotence (candidature doit être EN_ATTENTE,
--    sinon ré-acceptation → 2e contrat + 2e notif) ; (b) compat hiérarchique au lieu de
--    l'égalité stricte (candidatures IBODE→IDE étaient reçues mais impossibles à accepter) ;
--    (c) claim ATOMIQUE de la mission (WHERE statut='OUVERTE' + ROW_COUNT) déplacé AVANT
--    les UPDATE candidatures, pour qu'une course perdue ne laisse pas les candidatures
--    à moitié modifiées.
...
-- 2. fn_postuler_mission (ajout notif CANDIDATURE_RECUE à l'étab)
...
BEGIN
    ...
    -- Notifier l'établissement (parité avec le chemin swipe).
...
-- 3. fn_accepter_mission (compat hiérarchique au lieu de l'égalité stricte)
...
BEGIN
    ...
    -- Compatibilité hiérarchique (IBODE/IADE↔IDE, spécialité MEDECIN) — alignée sur
    -- fn_postuler_mission/fn_enregistrer_swipe (auparavant : égalité stricte).
...
-- 4. fn_traiter_candidature (garde idempotence + compat hiérarchique + claim atomique)
...
BEGIN
    ...
    -- Idempotence : ne traiter qu'une candidature encore en attente.
    ...
    IF p_decision = 'ACCEPTEE' THEN
        ...
        -- Claim ATOMIQUE de la mission AVANT de toucher les candidatures.
        UPDATE missions SET
