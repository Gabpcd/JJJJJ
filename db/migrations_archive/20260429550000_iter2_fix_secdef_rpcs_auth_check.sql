-- Itération 2 — 4 RPCs SECURITY DEFINER GRANTed à authenticated mais sans check auth
-- Risque détecté : escalade de privilèges (création factures/bulletins/notifs arbitraires + leak data biz)

-- 1) fn_admin_cohort_economics : ajouter check est_admin() (page admin uniquement)
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  -- Iter2 sec fix : page admin uniquement
  ...
  monthly_missions AS (
    SELECT TO_CHAR(cree_le, 'YYYY-MM') AS mois,
      ...
      COUNT(*) FILTER(WHERE statut LIKE 'ANNULEE%') AS missions_annulees,
  ...
  unit_eco AS (
    SELECT
      ...
      ROUND(COUNT(*) FILTER(WHERE statut LIKE 'ANNULEE%') * 100.0 / NULLIF(COUNT(*), 1), 1) AS taux_annulation
...
-- 2) fn_creer_notification : ajouter check auth.uid() (helper UI)
CREATE OR REPLACE FUNCTION public.fn_creer_notification(
  p_destinataire_id uuid, p_type_destinataire text, p_type text,
  p_titre text, p_corps text, p_lien text DEFAULT NULL,
  p_type_ressource text DEFAULT NULL, p_id_ressource uuid DEFAULT NULL
) RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
    v_id uuid;
BEGIN
    -- Iter2 sec fix : authentification requise
...
-- 3) fn_creer_bulletin_paie + fn_generer_facture_honoraires_mission : REVOKE authenticated
-- Ces fonctions sont appelées par triggers/cron uniquement (postgres + service_role suffisent)
-- Pas de référence dans le code TypeScript frontend.
REVOKE EXECUTE ON FUNCTION public.fn_creer_bulletin_paie(uuid) FROM authenticated;
REVOKE EXECUTE ON FUNCTION public.fn_generer_facture_honoraires_mission(uuid) FROM authenticated;
