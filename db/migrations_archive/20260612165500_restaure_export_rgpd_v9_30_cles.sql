-- Restaure la couverture article 15 RGPD de l'export soignant (contrat v9, ~30 clés).
--
-- Régression : 20260608120000_cleanup_fonctions_superseded_mortes.sql a supprimé
-- fn_exporter_mes_donnees() (v9, ~30 clés — cf. 20260429540000_iter1_fix_rgpd_export_messages.sql)
-- au motif « superseded par fn_rgpd_exporter_rate_limited ». Or ce wrapper délègue à
-- fn_rgpd_exporter_donnees_soignant(uid) qui ne retournait que 5 clés (soignant,
-- missions, documents, presences, export_date). L'export effectif côté app
-- (SectionConfidentialite.tsx) était donc passé de ~30 catégories de données à 4 —
-- non conforme art. 15 RGPD (droit d'accès à TOUTES les données personnelles traitées).
--
-- Fix : réécriture de fn_rgpd_exporter_donnees_soignant(p_soignant_id) avec le corps
-- v9 intégral, adapté :
--   * auth.uid() → p_soignant_id (le wrapper rate-limited passe v_user_id,
--     cf. 20260531139000) ;
--   * garde anti-IDOR : la fonction est exposée PostgREST avec un uid paramétrable —
--     un utilisateur authentifié ne peut exporter QUE ses propres données
--     (auth.uid() = p_soignant_id). service_role (auth.uid() NULL) reste libre
--     (support admin). + REVOKE anon/authenticated en défense en profondeur :
--     le flux app passe par le wrapper SECURITY DEFINER (owner postgres).
--   * favoris : la v9 lisait déjà favoris_soignant_etab (renommage acquis) ;
--   * audit : appel 6-args nommés → matche l'overload canonique 9-args-à-défauts
--     (le 6-args dédié a été DROPpé par 20260531131000).
-- Le wrapper public.fn_rgpd_exporter_rate_limited() reste INCHANGÉ.

DROP FUNCTION IF EXISTS public.fn_rgpd_exporter_donnees_soignant(uuid);

CREATE FUNCTION public.fn_rgpd_exporter_donnees_soignant(p_soignant_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Défense en profondeur : seul le wrapper (SECURITY DEFINER, owner postgres) et le
-- service_role appellent l'implémentation interne. Pas d'appel direct anon/authenticated.
REVOKE EXECUTE ON FUNCTION public.fn_rgpd_exporter_donnees_soignant(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_rgpd_exporter_donnees_soignant(uuid) TO service_role;
