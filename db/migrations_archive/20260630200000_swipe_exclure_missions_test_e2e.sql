-- B4 — Fuite de données de test E2E dans le SWIPE.
--
-- Le feed public (fn_missions_publiques_recherche) exclut déjà les missions de test
-- (intitule LIKE '[%', migration 20260630130000). Mais fn_obtenir_missions_swipe ne les
-- filtrait PAS : un vrai soignant qui swipe voyait les cartes « [playwright-test] … » et
-- « [pw-test:match] … » seedées en continu par la suite Playwright.
--
-- On les exclut du swipe SAUF pour le compte de test playwright-* : matching-backend.spec
-- seede et vérifie justement ses missions [playwright-test] via ce RPC. Le filtre
-- `OR v_sg.email LIKE 'playwright-%'` garde la CI E2E verte tout en masquant ces missions
-- à tout vrai utilisateur. (Isolation propre = projet Supabase de test dédié, backlog.)
...
-- Re-GRANT : CREATE OR REPLACE déclenche fn_auto_revoke_anon_execute (révoque anon).
