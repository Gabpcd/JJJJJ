...
const email = 'admin@jolene.app';
const candidates = [
  ['PLAYWRIGHT_ADMIN_PASSWORD', process.env.PLAYWRIGHT_ADMIN_PASSWORD_PRIMARY || ''],
  ['ADMIN_TEST_PASSWORD', process.env.PLAYWRIGHT_ADMIN_PASSWORD_FALLBACK || ''],
].filter(([, value]) => value);
...
for (const [name, password] of candidates) {
  ...
  await appendFile(githubEnv, `PLAYWRIGHT_ADMIN_PASSWORD<<JOLENE_ADMIN_EOF\n${password}\nJOLENE_ADMIN_EOF\n`);
...
console.error(
  'Aucun des secrets GitHub existants n’authentifie admin@jolene.app.',
);
process.exit(1);
